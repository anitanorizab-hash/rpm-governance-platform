// Executive Copilot page (CP20E): advisory leadership decision support (HITL; no direct approval).
...
export default function ExecutiveCopilot() {
  ...
  async function onGenerateBriefing() {
    ...
    setError(null);
    try {
      setBriefing(await copilotService.briefing());
  ...
  async function onAsk(e) {
    ...
    setError(null);
    try {
      setAnswer(await copilotService.ask(question.trim()));
      setHistory(await copilotService.history());
  ...
  async function onCreateRec(data) {
    ...
    try {
      setCreatedRec(await copilotService.createRecommendation(data));
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-slate-800">Executive Copilot</h1>
        <p className="text-sm text-slate-500">Advisory leadership decision support across KPIs, risk and budget.</p>
      </div>
